...
struct qtatom {
    /** The offset from the start of the file (or start of streaming
     * input) where this item starts (at the first length field). */
    uint64_t file_offset;

    /** The length of this object, either the first 4-byte length
     * field or the 8-byte extended length. */
    uint64_t length;

    /** The number of bytes remaining in this object as we continue
     * to parse enput */
    uint64_t remaining;

    /** The string like 'moov' or 'ftyp' that identifies this atom. This
     * is read in using big-endian mode, and should equal one of the 
     * values in the enum QTAtomType */
    uint32_t type;

    /** The hierarchical level, 0 for top level, 1 or next level, and
     * so on */
    unsigned indent;

    /** Either 8 or 16, depending upon whether this is a normal header
     * or an extended header */
